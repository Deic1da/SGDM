...
<a class="voltarInicio" href="{{ route('pontos-coleta.index') }}" aria-label="Voltar para lista de pontos de coleta">
...
<form action="{{ route('pontos-coleta.update', $entidade) }}" method="POST">
@csrf
@method('PUT')
...
<div class="grid">
    ...
    <div class="medicamento">
        ...
        <input id="razao-social" name="razao_social" type="text" placeholder="Digite a razao social" value="{{ old('razao_social', $entidade->razao_social) }}" required>

        <label for="nome-fantasia">Nome Fantasia</label>
        <input id="nome-fantasia" name="nome_fantasia" type="text" placeholder="Digite o nome fantasia" value="{{ old('nome_fantasia', $entidade->nome_fantasia) }}">

        <label for="cnpj">CNPJ</label>
        <input id="cnpj" name="cnpj" type="text" placeholder="XX.XXX.XXX/XXXX-XX" value="{{ old('cnpj', $entidade->cnpj) }}" required>

        <label for="horario-abertura">Horario de Funcionamento</label>
        <p class="exemplo">Selecione o horario de abertura e fechamento.</p>
        @php
            $horario = (string) old('horario_funcionamento', $entidade->horario_funcionamento);
        ...
        <input id="horario-funcionamento" name="horario_funcionamento" type="hidden" value="{{ old('horario_funcionamento', $entidade->horario_funcionamento) }}" required>

        <div class="check">
            <input id="aceita-validade-curta" name="aceita_validade_curta" type="checkbox" value="1" {{ old('aceita_validade_curta', $entidade->aceita_validade_curta) ? 'checked' : '' }}>
    ...
    <div class="data-validade">
        ...
        <input id="cep" name="cep" type="text" placeholder="XXXXX-XXX" value="{{ old('cep', $entidade->cep) }}" required>

        <label for="logradouro">Logradouro</label>
        <input id="logradouro" name="logradouro" type="text" placeholder="Rua, avenida, etc." value="{{ old('logradouro', $entidade->logradouro) }}" required>

        <label for="numero">Numero</label>
        <input id="numero" name="numero" type="text" placeholder="Numero" value="{{ old('numero', $entidade->numero) }}" required>

        <label for="bairro">Bairro</label>
        <input id="bairro" name="bairro" type="text" placeholder="Bairro" value="{{ old('bairro', $entidade->bairro) }}" required>

        <label for="municipio">Municipio</label>
        <input id="municipio" name="municipio" type="text" placeholder="Municipio" value="{{ old('municipio', $entidade->municipio) }}" required>

        <label for="estado">Estado</label>
        <input id="estado" name="estado" type="text" placeholder="UF" maxlength="2" value="{{ old('estado', $entidade->estado) }}" required>
    </div>
...
<button class="finalizar" type="submit">Salvar Alteracoes</button>
